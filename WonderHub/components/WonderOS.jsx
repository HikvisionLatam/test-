...
const SoftwareTooltip = ({ label, children }) => {
    return (
        <div 
            ...
            // Analytics: Hover en elementos del OS
            onMouseEnter={() => {
                trackEvent('view.help', {
                    click_chapter1: 'WonderHub',
                    click_chapter2: 'Interface',
                    click_name: 'hover_os_tooltip',
                    label: label || 'unknown'
                });
            }}
        >
...
const PianoApp = ({ onClose }) => {
    ...
    const playNote = (freq) => {
        // Analytics: Track Note Played
        trackEvent('click.action', {
            click_chapter1: 'WonderHub',
            click_chapter2: 'PianoApp',
            click_name: 'play_note',
            note_freq: Math.round(freq)
        });
    ...
    return (
        <div className="absolute inset-0 z-[2000] bg-[#333] flex flex-col items-center justify-center animate-fade-in">
            ...
            <div className="absolute bottom-0 w-full h-16 bg-[#222] border-t border-gray-700 flex items-center justify-between px-8">
                <button
                    onClick={() => {
                        // Analytics: Close Piano
                        trackEvent('click.navigation', {
                            click_chapter1: 'WonderHub',
                            click_chapter2: 'PianoApp',
                            click_name: 'close_app'
                        });
                        onClose();
                    }}
                ...
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-3 bg-black/40 px-4 py-2 rounded-full border border-gray-600">
                        ...
                        <input
                            ...
                            value={volume}
                            onChange={(e) => {
                                // Analytics: Change Volume (Solo al soltar o cada vez? Mejor simple por ahora)
                                // Nota: onChange en inputs range dispara muchos eventos, en prod usaríamos debounce.
                                // Aquí lo dejo pero ten en cuenta el volumen de datos.
                                setVolume(e.target.value);
                            }}
                            onMouseUp={() => {
                                // Trackeamos el volumen final al soltar
                                trackEvent('click.action', {
                                    click_chapter1: 'WonderHub',
                                    click_chapter2: 'PianoApp',
                                    click_name: 'change_volume',
                                    volume_level: volume
                                });
                            }}
...
const RandomSelectApp = ({ onClose }) => {
        ...
        if (step === 1 && pickerListRef.current) {
            ...
            Draggable.create(pickerListRef.current, {
                ...
                onDragEnd: function () {
                    ...
                    const newOptions = index + 2;
                    setNumOptions(newOptions);
                    // Analytics: Change Number of Options via Drag
                    trackEvent('click.action', {
                        click_chapter1: 'WonderHub',
                        click_chapter2: 'RouletteApp',
                        click_name: 'set_options_count',
                        options_count: newOptions
                    });
                }
            });
        }
    ...
    const spinWheel = () => {
        ...
        trackEvent('click.action', {
            click_chapter1: 'WonderHub',
            click_chapter2: 'RouletteApp',
            click_name: 'spin_wheel',
            options_count: numOptions
        });
        ...
        <div className="absolute inset-0 z-[2000] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in">
            <div className="bg-white w-[500px] h-[600px] rounded-[2rem] shadow-2xl flex flex-col relative overflow-hidden border border-white/20">
                <div className="flex justify-between items-center p-6 border-b border-gray-100">
                    ...
                    <button 
                        onClick={() => {
                            trackEvent('click.navigation', { click_chapter1: 'WonderHub', click_chapter2: 'RouletteApp', click_name: 'close_app' });
                            onClose();
                        }} 
                    ...
                    {step === 1 && (
                        <div className="w-full flex flex-col items-center animate-fade-in">
                            ...
                            <div className="w-full px-10 mt-10 flex gap-4">
                                <button 
                                    onClick={() => {
                                        trackEvent('click.action', { click_chapter1: 'WonderHub', click_chapter2: 'RouletteApp', click_name: 'cancel_setup' });
                                        onClose();
                                    }} 
                                    className="flex-1 py-4 text-gray-500 hover:bg-gray-100 rounded-xl font-bold text-lg transition-colors cursor-pointer"
                                >{t('roulette.cancel')}</button>
                                <button 
                                    onClick={() => {
                                        setStep(2);
                                        trackEvent('click.navigation', { click_chapter1: 'WonderHub', click_chapter2: 'RouletteApp', click_name: 'go_to_wheel' });
                                    }} 
                                    className="flex-1 py-4 bg-blue-600 text-white rounded-xl font-bold text-lg shadow-lg hover:bg-blue-700 hover:shadow-xl hover:-translate-y-1 transition-all cursor-pointer"
                                >{t('roulette.next')}</button>
                            </div>
                        </div>
                    )}
                    {step === 2 && (
                    ...
                    {step === 3 && (
                        <div className="flex flex-col items-center justify-center w-full h-full animate-scale-up">
                            ...
                            <div className="flex gap-4">
                                <button 
                                    onClick={() => { 
                                        trackEvent('click.navigation', { click_chapter1: 'WonderHub', click_chapter2: 'RouletteApp', click_name: 'restart_config' });
                                        setStep(1); setSpinning(false); setWinner(null); 
                                    }} 
                                    className="px-6 py-3 text-gray-500 font-medium hover:bg-gray-100 rounded-xl transition-colors cursor-pointer"
                                >{t('roulette.configure')}</button>
                                <button 
                                    onClick={() => { 
                                        trackEvent('click.navigation', { click_chapter1: 'WonderHub', click_chapter2: 'RouletteApp', click_name: 'spin_again' });
                                        setStep(2); setSpinning(false); setWinner(null); 
                                    }} 
                                    className="px-8 py-3 bg-blue-600 text-white font-bold rounded-xl shadow-lg hover:bg-blue-700 hover:shadow-xl transition-all cursor-pointer"
                                >{t('roulette.spinAgain')}</button>
                            </div>
                        </div>
                    )}
                </div>
...
const ToolsMenu = ({ isOpen, onClose, onLaunchApp, showToast }) => {
    ...
    const tools = [
        { name: t('tools.names.note'), icon: <StickyNote className="text-blue-500" />, action: 'demo', id: 'tool_note' },
        { name: t('tools.names.camera'), icon: <Camera className="text-gray-600" />, action: 'demo', id: 'tool_camera' },
        { name: t('tools.names.screenshot'), icon: <Crop className="text-gray-600" />, action: 'demo', id: 'tool_screenshot' },
        { name: t('tools.names.calculator'), icon: <Calculator className="text-gray-600" />, action: 'demo', id: 'tool_calculator' },
        { name: t('tools.names.stopwatch'), icon: <Timer className="text-gray-600" />, action: 'demo', id: 'tool_stopwatch' },
        { name: t('tools.names.countdown'), icon: <Hourglass className="text-gray-600" />, action: 'demo', id: 'tool_countdown' },
        { name: t('tools.names.voting'), icon: <CheckSquare className="text-gray-600" />, action: 'demo', id: 'tool_voting' },
        { name: t('tools.names.random'), icon: <Dices className="text-orange-500 glow-cta border-glow" />, action: 'random', id: 'tool_random' },
        { name: t('tools.names.piano'), icon: <Music className="text-purple-500 glow-cta border-glow" />, action: 'piano', id: 'tool_piano' },
        { name: t('tools.names.record'), icon: <Video className="text-gray-600" />, action: 'demo', id: 'tool_record' },
        { name: t('tools.names.lock'), icon: <Lock className="text-gray-600" />, action: 'demo', id: 'tool_lock' },
        { name: t('tools.names.transfer'), icon: <ArrowRightLeft className="text-gray-600" />, action: 'demo', id: 'tool_transfer' },
        { name: t('tools.names.remote'), icon: <Smartphone className="text-gray-600" />, action: 'demo', id: 'tool_remote' },
    ];
            ...
            <div className="grid grid-cols-3 gap-4">
                {tools.map((tool, i) => (
                    <div
                        ...
                        onClick={() => {
                            // Analytics: Launch Tool from Menu
                            trackEvent('click.navigation', {
                                click_chapter1: 'WonderHub',
                                click_chapter2: 'ToolsMenu',
                                click_name: 'launch_tool',
...
const FolderIcon = ({ label, onClick, apps }) => {
    return (
        <div 
            className="flex flex-col items-center gap-2 group cursor-pointer app-anim opacity-0 transform translate-y-4" 
            onClick={() => {
                // Analytics: Open Folder
                trackEvent('click.navigation', {
                    click_chapter1: 'WonderHub',
                    click_chapter2: 'Desktop',
                    click_name: 'open_folder',
                    folder_label: label
                });
                onClick();
            }}
...
const WonderOS = () => {
    ...
    const handleSearch = (e) => { 
        if (e.key === 'Enter') { 
            ...
            trackEvent('click.action', {
                click_chapter1: 'WonderHub',
                click_chapter2: 'Desktop',
                click_name: 'submit_search',
                keyword_length: searchText.length // No guardamos PII (texto real), solo metadata
            });
    ...
    const handleAppClick = (appName) => {
        // Analytics: Launch Main Apps
        trackEvent('click.navigation', {
            click_chapter1: 'WonderHub',
            click_chapter2: 'Desktop',
            click_name: 'launch_app',
            app_name: appName
        });
            ...
            {showIntro && (
                <div className="absolute inset-0 z-[3000] bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center text-center px-6">
                    <div className="max-w-xl bg-white/95 rounded-3xl shadow-2xl p-8 space-y-4 animate-scale-up">
                        ...
                        <button
                            onClick={() => {
                                // Analytics: Dismiss Intro
                                trackEvent('click.action', {
                                    click_chapter1: 'WonderHub',
                                    click_chapter2: 'Intro',
                                    click_name: 'start_experience'
                                });
                                setShowIntro(false);
                            }}
            ...
            <div className="absolute bottom-0 left-0 w-full h-14 bg-gray-900/90 backdrop-blur-md flex items-center justify-between px-6 z-[2000] border-t border-white/10 text-white/90 shadow-2xl">
                <div className="flex items-center gap-3">
                    <SoftwareTooltip label={t('bar.input')}>
                        <div 
                            className="flex items-center gap-2 px-3 py-1 rounded-lg hover:bg-white/10 transition-colors cursor-pointer" 
                            onClick={() => {
                                trackEvent('click.action', { click_chapter1: 'WonderHub', click_chapter2: 'Taskbar', click_name: 'change_source' });
                                showToast(t('os.sourceHdmi'));
                            }}
                        >
                            <Monitor size={18} />
                ...
                <div className="absolute left-1/2 transform -translate-x-1/2 flex items-center gap-16">
                    <SoftwareTooltip label={t('bar.allApps')}>
                        <button 
                            onClick={() => trackEvent('click.navigation', { click_chapter1: 'WonderHub', click_chapter2: 'Taskbar', click_name: 'open_app_grid' })}
                            className="hover:bg-white/20 p-2 rounded-lg transition-colors"
                        >
                            <LayoutGrid size={22} />
                        </button>
                    </SoftwareTooltip>
                    <SoftwareTooltip label={t('bar.home')}>
                        <button 
                            onClick={() => {
                                trackEvent('click.navigation', { click_chapter1: 'WonderHub', click_chapter2: 'Taskbar', click_name: 'go_home' });
                                setActiveApp(null);
                            }} 
                            className="bg-white/20 p-2 rounded-lg shadow-inner text-white cursor-pointer"
                        >
                            <Home size={22} />
                        </button>
                    </SoftwareTooltip>
                    <SoftwareTooltip label={t('bar.multitask')}>
                        <button 
                            onClick={() => trackEvent('click.navigation', { click_chapter1: 'WonderHub', click_chapter2: 'Taskbar', click_name: 'open_multitask' })}
                            className="hover:bg-white/20 p-2 rounded-lg transition-colors"
                        >
                            <Layers size={22} />
                        </button>
                ...
                <div className="flex items-center gap-6">
                    <SoftwareTooltip label={t('bar.wifi')}>
                        <Wifi 
                            size={20} 
                            className="cursor-pointer hover:text-blue-400" 
                            onClick={() => trackEvent('click.action', { click_chapter1: 'WonderHub', click_chapter2: 'Taskbar', click_name: 'open_wifi_settings' })}
                        />
                    </SoftwareTooltip>
                    <SoftwareTooltip label={t('bar.quickSettings')}>
                        <SlidersHorizontal 
                            size={20} 
                            className="cursor-pointer hover:text-blue-400" 
                            onClick={() => trackEvent('click.action', { click_chapter1: 'WonderHub', click_chapter2: 'Taskbar', click_name: 'open_quick_settings' })}
                        />
                    </SoftwareTooltip>
                    <div className="relative">
                        <SoftwareTooltip label={t('bar.tools')}>
                            <Briefcase
                                ...
                                `}
                                onClick={() => {
                                    trackEvent('click.action', { click_chapter1: 'WonderHub', click_chapter2: 'Taskbar', click_name: 'toggle_tools_menu', state: !isToolsMenuOpen ? 'open' : 'close' });
