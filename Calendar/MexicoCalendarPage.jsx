    ...
    useEffect(() => {
        const fetchEvents = async () => {
            try {
                ...
                // Filtrar eventos de México Y eventos Destacados/LATAM
                const mexicoAndFeaturedEvents = rows.filter(row => {
                    const isMexico = row.name === 'México' || row.name === 'Mexico';
                    const isFeatured = row.typeEvent === 'Destacados' || row.name === 'LATAM' || row.name === 'Latam';
                    return isMexico || isFeatured;
                }).map(row => {
                ...
                setEventsByCountry({ México: mexicoAndFeaturedEvents });
            } catch (err) {
