...
const Calendar = ({ events, availableTabs, defaultTab }) => {
    ...
    const [activeTab, setActiveTab] = useState(defaultTab || (availableTabs?.[0] || null));
    ...
    // REF PARA SPLIDE (PAÍSES)
    const splideRef = useRef(null);

    const [contentHeight, setContentHeight] = useState('auto');

    useEffect(() => {
        if (!splideRef.current || !activeTab || availableTabs.length === 0) return;
        
        const index = availableTabs.indexOf(activeTab);
        if (index === -1) return;

        // Usar setTimeout para asegurar que Splide está completamente inicializado
