...
${headerMsg}
🚀 ¡${event.titulo}!
🔍 ${event.description ? event.description : t('calendar.dont_miss')}
🗓 ${event.dia} ${t('calendar.date_of')} ${monthTranslated}
...
👉 ${event.link_registro}
        ...
        if (ok) {
            setTooltipMsg(t('calendar.copied'));
            setTimeout(() => setTooltipMsg(''), 2000);
        } else {
            setTooltipMsg(t('calendar.copy_failed'));
    ...
    const processedTimelineEvents = useMemo(() => {
        ...
        let timelineItems = filteredTodayEvents.map(event => {
            ...
            const height = Math.max(end - top, ROW_HEIGHT / 2);
            return {
                ...event,
                top,
                bottom: top + height,
        ...
        clusters.forEach(cluster => {
            ...
            cluster.forEach(event => {
                event.style = {
                    ...
                    left: `calc(${colWidth * event.colIndex}% + ${event.colIndex > 0 ? 5 : 0}px)`,
                    width: `calc(${colWidth}% - 10px)`,
                    position: 'absolute',
                    zIndex: 20 + event.colIndex,
    ...
    const agendaGroups = useMemo(() => {
        ...
        return Object.keys(groups).sort().map(time => ({
            time,
            events: groups[time],
            dotColors: groups[time].map(e => e.tipo_evento === 'Destacados' ? 'destacado' : (eventColorMap[e.tipo_evento] || eventColorMap.default))
        }));
    }, [filteredTodayEvents, viewMode]);

    const renderCardContent = (event, isAgenda = false) => {
        const isDestacado = event.tipo_evento === 'Destacados';
        ...
        const actionLabel = t('calendar.register');

        const isVirtual = event.modalidad === 'Virtual';
        const modalityColor = isVirtual ? MODALITY_COLORS['Virtual'] : MODALITY_COLORS['Presencial'];

        return (
            <div className={`flex flex-col h-full relative ${isDestacado ? 'bg-white/95' : ''}`} style={{ padding: '16px' }}>
                ...
                <div className={`grid grid-cols-2 gap-x-[10px] gap-y-[5px] ${detailSize} text-gray-600 mb-[10px]`}>
                    <div className="flex flex-col gap-[5px]">
                        ...
                        <div className="flex items-center gap-[6px]">
                            <i className={`fas ${event.modalidad === 'Virtual' ? 'fa-video' : 'fa-building'} w-[14px] text-center text-gray-400`}></i>
                            <span className="truncate">{event.modalidad === 'Virtual' ? t('calendar.online') : (event.lugar || t('calendar.presencial'))}</span>
                        </div>
                    </div>
                    <div className="flex flex-col gap-[5px]">
                        <div className="flex items-center gap-[6px]">
                            <i className="fas fa-globe w-[14px] text-center text-gray-400"></i>
                            <span className="truncate">{event.lenguaje}</span>
                        </div>
                    </div>
                </div>
                        ...
                        <a
                            ...
                            onClick={(e) => {
                                ...
                                trackEvent('click.exit', {
                                    click_chapter1: 'calendario_eventos',
                                    click_chapter2: 'vista_dia',
                                    click_name: 'ir_registro_',
                                    event_id: event.id,
                                    event_title: event.titulo
                                });
                            }}
                            className={`flex-1 text-white text-xs font-bold py-2 px-4 rounded-lg text-center transition-opacity hover:opacity-90 shadow-sm ${isDestacado ? 'bg-destacados-gradient border-0' : ''}`}
                            style={{ backgroundColor: isDestacado ? undefined : baseColor }}
                        >
                            {actionLabel}
                        </a>
                        <button
            ...
            <div className="flex flex-col p-[21px] pb-[10px] sticky top-0 z-40 bg-gray-50/95 backdrop-blur-sm border-b border-gray-100">
                <div className="flex justify-between items-center mb-[10px]">
                    <h3 className="text-xl font-bold font-figtree text-gray-800 mb-0">
                        {capitalize(format(date, "eeee, dd 'de'", { locale: currentLocale }))} {capitalize(format(date, 'LLLL', { locale: currentLocale }))}
                    </h3>
                    <div className="flex items-center gap-3">
                        <div className="flex bg-gray-100 p-1 rounded-lg">
                            ...
                            >
                                <i className="fas fa-list-ul mr-1"></i> {t('calendar.agenda')}
                            </button>
                            <button
                                onClick={() => {
                                    trackEvent('click.action', {
                            ...
                            >
                                <i className="fas fa-clock mr-1"></i> {t('calendar.timeline')}
                            </button>
                        </div>
                        <button className="p-[8px] rounded-full hover:bg-gray-100 text-gray-500 transition-colors cursor-pointer" onClick={() => {
                            trackEvent('click.action', {
                                click_chapter1: 'calendario_eventos',
                                click_chapter2: 'vista_dia',
                                click_name: 'cerrar_vista_dia'
                            });
                            onClose();
                        }}>
                            ...
                            <button
                                ...
                                onClick={() => {
                                    trackEvent('click.action', {
                                        ...
                                        filter_value: type,
                                        filter_active: dayFilter === type ? 'desactivar' : 'activar'
                                    });
                                    setDayFilter(prev => prev === type ? null : type);
                                }}
                                    ...
                                    <InteractionCard
                                        ...
                                        onClick={() => {
                                            trackEvent('click.navigation', {
                                                click_chapter1: 'calendario_eventos',
                                                click_chapter2: 'vista_dia',
                                                click_name: 'abrir_modal_evento_timeline',
                                                event_id: event.id,
                                                event_title: event.titulo
                                            });
                                            setSelectedEvent({ ...event, country: currentCountry });
                                        }}
                                        cursorText={t('calendar.register')}
                    ...
                    <div className="p-6 max-w-4xl mx-auto w-full">
                        {agendaGroups.length === 0 ? (
                            <div className="flex flex-col items-center justify-center h-64 text-gray-400">
                                ...
                                <p>{t('calendar.no_events_filter')}</p>
                            </div>
                        ) : (
                            agendaGroups.map((group, groupIndex) => (
                                <div key={group.time} className="flex gap-6 mb-8 relative">
                                    <div className="hidden md:flex w-20 flex-col items-end pt-1 relative shrink-0">
                                        <span className="text-base font-bold text-gray-800 leading-none">
                                            {formatTimeAMPM(group.time)}
                                        </span>
                                        <div className="absolute right-[-34px] top-3 flex gap-1 z-10 bg-gray-50 py-1">
                                    ...
                                    <div className="flex-1 md:pl-10 lg:pl-10">
