...
import { 
    ...
    addDays, subDays, addWeeks, subWeeks, isSameMonth, addMonths, subMonths, isBefore, getDate
} from 'date-fns';
...
const MonthView = ({ 
    ...
    useEffect(() => {
        if (selectedDate) {
            setCurrentDate(prev => isSameMonth(prev, selectedDate) ? prev : selectedDate);
        }
    }, [selectedDate]);
    ...
    const initialSlideIndex = useMemo(() => {
        const targetDate = selectedDate && isSameMonth(selectedDate, currentDate) ? selectedDate : new Date();
        const foundIndex = daysInMonth.findIndex(d => getDate(d) === getDate(targetDate));
    ...
    }, [currentDate, initialSlideIndex, activeFilter]); 
    ...
    const renderDayButton = (day, isMobile = false) => {
        ...
        // --- LOGICA DE PRIORIDAD: Verificar si hay eventos Destacados ---
        const hasFeaturedEvent = uniqueTypes.includes('Destacados');
        ...
        const getDynamicStyles = () => {
            ...
            // --- AQUÍ APLICAMOS LA PRIORIDAD ---
            // Si el día tiene un evento Destacado, este estilo PREDOMINA sobre los demás.
            if (hasFeaturedEvent) {
                // Retornamos flag para activar la clase CSS de la animación
                return { isDestacado: true, color: '#FFFFFF' }; 
            }
            
            // Lógica estándar para eventos normales
        ...
        const isDestacadoStyle = styles.isDestacado;

        const getOpacity = () => {
            if (!activeFilter || !hasEvents || isDayDisabled) return 1;
            // Si filtramos, los destacados también deberían respetar el filtro si se selecciona 'Destacados'
        ...
        const baseClasses = `
            relative flex flex-col items-center justify-center font-bold transition-all duration-200
            ${isMobile ? 'w-12 h-14 rounded-2xl text-lg' : 'w-10 h-10 rounded-full text-sm'}
            ${isDayDisabled ? 'cursor-not-allowed' : (hasEvents ? 'cursor-pointer shadow-sm hover:scale-105' : 'cursor-default')}
        ...
        return (
            <button
                ...
                // Si es destacado, agregamos la clase de animación. Si no, solo baseClasses.
                className={`${baseClasses} ${isDestacadoStyle ? 'bg-destacados-gradient' : ''}`}
                style={{
                    // Si es destacado, la clase CSS maneja el background. Si no, usamos el style inline.
                    background: isDestacadoStyle ? undefined : styles.background,
                    color: styles.color,
                    opacity: getOpacity(),
                }}
                ...
                onClick={!isDayDisabled && hasEvents ? () => {
                    trackEvent('click.navigation', {
                        click_chapter1: 'calendario_eventos',
                        click_chapter2: isMobile ? 'vista_mes_movil' : 'vista_mes',
                ...
                onMouseEnter={() => hasEvents && !isMobile && setHoveredDay(dayKey)}
                onMouseLeave={() => !isMobile && setHoveredDay(null)}
            >
                {isMobile && (
                    <span className="text-[10px] uppercase font-bold opacity-70 mb-1" style={{ color: 'inherit' }}>
                ...
                {hasEvents && !isDayDisabled && (
                    <span 
                        className={`absolute ${isMobile ? '-top-2 -right-2 w-5 h-5 text-[10px]' : '-top-1 -right-1 w-4 h-4 text-[9px]'} bg-white text-gray-800 font-extrabold rounded-full flex items-center justify-center border border-gray-100 shadow-sm z-20`}
                        aria-hidden="true"
                    >
                        {eventCount}
                    </span>
    ...
    return (
        <div className="p-4 md:p-6 bg-white rounded-2xl shadow-sm month-view-wrapper font-figtree border border-gray-100 relative">
            ...
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-xl md:text-2xl font-bold font-figtree text-gray-800 m-0 leading-none">
                    ...
                    {currentMonthEventTypes.map(type => (
                        ...
                        >
                            <div
                                ...
                                // Si es Destacado, mostramos su gradiente en el filtro también (opcional) o el color base morado
            ...
            <div className="block md:hidden pb-2 w-full">
                <Splide
                    ref={splideRef}
                    key={`splide-${format(currentDate, 'yyyy-MM')}`}
                    options={{
                        fixedWidth: '4rem', 
                        focus: 0, 
                        gap: '0.2rem',
                    ...
                    className="calendar-day-slider"
                    style={{ visibility: 'visible' }}
                >
                    {daysInMonth.map(day => (
                        <SplideSlide key={day.toString()} className="flex justify-center items-center py-2">
                            {renderDayButton(day, true)}
                        </SplideSlide>
                    ))}
                </Splide>
