...
const MonthView = ({ 
    ...
    // Estado para el mensaje de bloqueo de navegación
    ...
    // Lógica para bloquear mes anterior
    const handlePrevMonth = () => {
        const prevMonthDate = subMonths(currentDate, 1);
        const todayStartOfMonth = startOfMonth(new Date());

        // Si el mes anterior es antes del mes actual real, bloqueamos
        if (isBefore(prevMonthDate, todayStartOfMonth)) {
            setNavTooltip({ show: true, msg: 'No se pueden visualizar eventos de meses anteriores.' });
    ...
    const eventsByDay = useMemo(() => {
        ...
        events.forEach(event => {
            ...
            if (monthIndex !== undefined && !isNaN(day)) {
                // Filtramos para que solo entren eventos del mes que estamos visualizando
                // Esto es crucial para que los filtros funcionen solo sobre lo visible
    ...
    // Calcular Tipos de Eventos Únicos SOLO del mes actual
    ...
    return (
        <div className="p-6 bg-white rounded-2xl shadow-sm month-view-wrapper font-figtree border border-gray-100 relative">
            ...
            {/* Tooltip de navegación bloqueada */}
            {navTooltip.show && (
            ...
            {/* --- HEADER: Icono, Título y Navegación --- */}
            <div className="flex items-center justify-between mb-2">
                {/* Lado Izquierdo: Icono + Mes Año */}
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-2xl font-bold font-figtree text-gray-800 m-0 leading-none">
                        {capitalize(format(currentDate, 'LLLL', { locale: es }))} <span className="text-gray-400 font-medium">{currentDate.getFullYear()}</span>
                    </h2>
                </div>

                {/* Lado Derecho: Flechas de Navegación */}
            ...
            {/* --- FILTROS (Basados en el mes actual) --- */}
            ...
            <div 
                ...
                role="grid"
                aria-label={`Calendario de ${format(currentDate, 'LLLL yyyy', { locale: es })}`}
            ...
            >
                ...
                {daysInMonth.map(day => {
                    ...
                    const isDayDisabled = isPastDay || allEventsAreDisabled;
                    ...
                    const getDynamicStyles = () => {
                        ...
                        // Gradiente si hay múltiples tipos
                        const backgroundColors = uniqueTypes.map(type => eventColorMap[type] || eventColorMap.default);
                        let backgroundStyle;
                        if (backgroundColors.length === 1) {
                            backgroundStyle = { background: backgroundColors[0] };
                        } else {
                            backgroundStyle = { background: `linear-gradient(135deg, ${backgroundColors.join(', ')})` };
                        }
                        const textColor = uniqueTypes.length === 1 ? eventTextColorMap[uniqueTypes[0]] || '#FFFFFF' : '#FFFFFF';
                        return { ...backgroundStyle, color: textColor };
                    };
                    ...
                    return (
                        <div key={day.toString()} role="gridcell" className="day-cell-wrapper relative flex justify-center py-1">
                            <button
                                ...
                                onClick={!isDayDisabled && hasEvents ? () => onDayClick(day) : undefined}
                            ...
                            >
                                ...
                                {hasEvents && (
                                    <div className={`calendar-tooltip${hoveredDay === dayKey ? ' show' : ''}`}>
                                        {isDayDisabled 
                                            ? `${eventCount} Finalizado${eventCount > 1 ? 's' : ''}`
