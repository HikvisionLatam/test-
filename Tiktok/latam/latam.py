...
async def hover_and_get_views(page, element):
    try:
        ...
        return 'N/A'
async def safe_goto(page, url, attempts=3, timeout=60000, wait_until='domcontentloaded'):
    """Navega con reintentos y timeout mayor. Por defecto usa 'domcontentloaded'."""
    for i in range(attempts):
        try:
            await page.goto(url, timeout=timeout, wait_until=wait_until)
            return
        except Exception as e:
            logging.warning(f"Goto falló ({i+1}/{attempts}): {e}")
            await asyncio.sleep(2 * (i+1))
    # Intento final (permitirá propagar la excepción si falla)
    await page.goto(url, timeout=timeout, wait_until=wait_until)
# ─── METADATOS + MINIATURA ──────────────────────────────────────────────────────
async def extract_video_info(page, video_url, views):
    await safe_goto(page, video_url); await random_sleep(2, 4); await handle_captcha(page)
    if views == 'N/A':
    ...
    desc = await page.evaluate(
        r'''() => {
            let d = '', tags = [];
            // Buscar selectores posibles (desc-span* es la nueva convención que vimos)
            const sels = ['span[data-e2e^="desc-span"]', 'span[data-e2e*="desc-span"]', 'span[data-e2e="new-desc-span"]'];
            let nodes = [];
            for (const s of sels) {
...
async def scrape_and_upload():
    ...
    async with async_playwright() as p:
        ...
        # aumentar timeouts y evitar fallos por espera de networkidle
        context.set_default_navigation_timeout(60000)
        context.set_default_timeout(60000)
        # extraer vistas
        page = await context.new_page()
        await safe_goto(page, f'https://www.tiktok.com/@{USERNAME}')
        await handle_captcha(page)
        for _ in range(36): await scroll_page(page); await handle_captcha(page)
