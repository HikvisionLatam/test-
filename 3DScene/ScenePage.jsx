...
const ScenePage = () => {
    ...
    return (
        <div style={{ width: '100vw', height: '100vh', background: '#ffffff', position: 'relative', overflow: 'hidden', fontFamily: "'Gilroy', sans-serif" }}>

            <div id="ui-layer" style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', pointerEvents: 'none', zIndex: 100 }}>
                <div style={{
                    position: 'absolute', pointerEvents: activePinId ? 'auto' : 'none',
                    opacity: activePinId ? 1 : 0, transition: 'all 0.5s cubic-bezier(0.2, 0.8, 0.2, 1)', zIndex: 101,
                    ...(isMobile ? {
                        bottom: activePinId ? '0px' : '-100%', left: '0px', width: '100%', display: 'flex', justifyContent: 'center', paddingBottom: '20px'
                    } : {
                        top: '50%', left: '40px', transform: 'translateY(-50%)',
                    })
                }}>
                    {activePinId && activePinData && (
                        <div style={{ width: isMobile ? '95%' : 'auto', maxWidth: '400px' }}>
                            <SwappingCard pinData={activePinData} onClose={() => setActivePinId(null)} />
                        </div>
                    )}
                </div>
            </div>

            <div id="canvas-layer" style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', zIndex: 1 }}>
