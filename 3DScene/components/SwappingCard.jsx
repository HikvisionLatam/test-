// 3DScene/components/SwappingCard.jsx
...
// Añadimos 'onProductSelect' a las props
const SwappingCard = ({ pinData, initialProductId, openedVia, onProductSelect, onClose }) => {
  ...
  useEffect(() => {
    setSelectedId(initialProductId || null);
  }, [pinData, initialProductId]);
  ...
  const toggleView = (id) => {
    ...
    // MAGIA: Avisamos al componente padre (ScenePage) para que mueva la cámara
    if (onProductSelect) {
        onProductSelect(id);
    }
  };
        ...
        {selectedId && selectedProduct && (
          <div className="detail-content">
            <div className='flex items-center justify-start gap-4 p-4 border-b-[1px] border-gray-200'>
              <button 
                className="close-btn-grid" 
                onClick={openedVia === 'product' ? onClose : () => toggleView(null)} 
                title={openedVia === 'product' ? "Cerrar" : "Volver al listado"}
              >
                {openedVia === 'product' ? '✕' : '←'}
              </button>
            </div>
